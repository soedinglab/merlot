...
def length_new_branch(NewEndPointCheck, EndPointsAux, DijkstraMatrix, DijkstraPredecesors, DijkstraSteps):
    ...
    branchingaux, TreeConnectivityaux = calculate_branchpoints(
        EndPointsAux, DijkstraMatrix, DijkstraPredecesors, DijkstraSteps)
    ...
    for i in range(0, TreeConnectivityaux.shape[0]):
        path = np.array(calculate_1cell_paths(
            TreeConnectivityaux[i][0], TreeConnectivityaux[i][1], DijkstraPredecesors))
    ...
    for i in range(0, Coordinates.shape[0]):
        if i not in ScaffoldCells:
            ...
            for j in range(0, len(ScaffoldCells)):
                DistancesScaffold[j] = (
                    euclidean(Coordinates[i], Coordinates[int(ScaffoldCells[j])]))
...
def calculate_secondary_endpoints(Endpoints, DijkstraMatrix, NumberNodes, NBranches=-1, BranchMinLength=-1, BranchMinLengthSensitive=-1, graph='no'):
    ...
    while TryEndpoint:
        ...
        new_endpoint = nodes_maxnodes[np.where(
            ScoreEndpoints[nodes_maxnodes] == np.max(ScoreEndpoints[nodes_maxnodes]))[0][0]]
        ...
        if NBranches > 0:
                ...
                if ScoreEndpointsNodes[new_endpoint] < BranchMinLength:
                        if BranchMinLengthSensitive < 0:
                            ...
                            LengthNewBranch = length_new_branch(
                                new_endpoint, EndPointsAux, DijkstraMatrix, DijkstraPredecesors, DijkstraSteps)
...
DijkstraMatrix, DijkstraSteps, DijkstraPredecesors = spm.shortest_path(
    InputMatrix, method="D", return_predecessors=True, directed=False)
DijkstraSteps = DijkstraSteps - 1

#Calculate Endpoints in the tree
EndPoints = calc_extreme_endpoints(DijkstraSteps)
Epsilons = calculate_secondary_endpoints(
