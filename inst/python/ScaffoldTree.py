...
def length_new_branch(NewEndPointCheck, EndPointsAux, DijkstraMatrix,
    ...
    # This function checks the length of a new branch that does not fulfill the
    # sqrt(N/2) requirement for a branch to be a valid one. It calculates the
    # topology structure of the tree by adding that new branch and calculates
    # the length of the new branch. Here we check to which branch the new endpoint belongs.
    EndPointsAux.append(NewEndPointCheck)
    branchingaux, TreeConnectivityaux = calculate_branchpoints(EndPointsAux, DijkstraMatrix,
                                                               DijkstraPredecesors, DijkstraSteps)
    ...
    for i in range(0, TreeConnectivityaux.shape[0]):
        path = np.array(calculate_1cell_paths(TreeConnectivityaux[i][0],
                                              TreeConnectivityaux[i][1], DijkstraPredecesors))
    ...
    # We check which cell i is mapped to the branch to which the new endpoint
    # belongs and add a +1 if TRUE
...
def calculate_secondary_endpoints(EndPoints, DijkstraMatrix, NumberNodes, NBranches=-1,
                                  BranchMinLength=-1, BranchMinLengthSensitive=-1, graph='no'):
    R_epsilon_todos = {}
    TryEndpoint = True
    NewBranch = True
    ...
    while TryEndpoint:
            ...
            if n in EndPoints:
                ...
                for k in range(0, len(EndPoints)):
                    for l in range(0, len(EndPoints)):
                        if k < l:
                            AddedDistance = DijkstraMatrix[n][EndPoints[k]]   \
                                            + DijkstraMatrix[n][EndPoints[l]] \
                                            - DijkstraMatrix[EndPoints[k]][EndPoints[l]]
                            AddedDistanceNodes = NumberNodes[n][EndPoints[k]]   \
        ...
        if np.max(ScoreEndpointsNodes) <= 0:
            print("[Note: EndPoints search terminated because no \
                   further positive scores were found...]")
            break
        ...
        if NBranches > 0:
            ...
            if ScoreEndpointsNodes[new_endpoint]<BranchMinLength:
                if BranchMinLengthSensitive < 0:
                    ...
                    LengthNewBranch = length_new_branch(new_endpoint, EndPointsAux,
                                                        DijkstraMatrix, DijkstraPredecesors,
                                                        DijkstraSteps)
...
def main():
    ...
    parser.add_argument('-NBranches', type=int, default=(-1), help='Number of desired \
                        branches for the resulting tree. By default the value is set \
                        to -1, meaning that all branches longer than sqrt(N/2) will be \
                        considered')
    parser.add_argument('-BranchMinLength', type=int, default=(-1), help='Minimum length \
                        for a branch to be included in the tree. Considers only cells in \
                        the scaffold as length. This threshold is quite strict. For a \
                        more sensitive detection a second threshold can be used with the \
                        option BranchMinLengthSensitive. Default minimum length: \
                        sqrt(N/2) with N being the number of cells in the dataset')
    parser.add_argument('-BranchMinLengthSensitive', type=int, default=(-1),
                        help='Minimum length for a branch to be included in the tree. \
                        It reconstructs the topology of the tree and maps cells to \
                        the potential new branch to decide if the branch will be \
                        added or not. Suggested value: sqrt(N) with N being the number \
                        of cells in the dataset')
    parser.add_argument('-showplot', type=str, default="no", help='Weather or not \
                        2D/3D plots should be shown. Options are "yes" and "no". \
    ...
    DijkstraMatrix, DijkstraSteps, DijkstraPredecesors = spm.shortest_path(InputMatrix,
                                                                           method="D",
    ...
    Epsilons = calculate_secondary_endpoints(EndPoints, DijkstraMatrix, DijkstraSteps,
    ...
    if len(EndPoints) <= 2: 
        ...
        with open(TreeTopologyDat, 'w') as out:
            ...
            for i in range(TreeConnectivity.shape[0]):
                # In case of trifurcations or higher order connections it avoids
                # printing branches in between the same node
                if TreeConnectivity[i][0] != TreeConnectivity[i][1]:
                    ...
                    TopologyLine = "Tree_Branch" + "\t" + str(TreeConnectivity[i][0]) \
                    + "\t" + str(TreeConnectivity[i][1]) + "\n"
                    out.write(TopologyLine)
        out.close()
